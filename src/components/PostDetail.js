            ...
            >
                ...
                <Flex styles={{flexDirection:"column"}}>
                    <Flex styles={{flexDirection:"column", gap:"5px", padding:"0 10px", alignItems:"flex-start"}}>
                        <Text styles={{fontSize:"28px", fontWeight:"800", alignItems:"start"}}>title</Text>
                        <Text>카테고리</Text>
                        <Flex styles={{justifyContent:"space-between"}}>
                            <Text>가격 : </Text>
                            <Text styles={{margin:"0 160px 0 0"}}>기한 : </Text>
                        </Flex>
                        <Text>내용 : 위 예제는 axios를 사용하기 위해 import 한 후 axios.defaults 속성 중 하나인 baseURL을 jsonplaceholder 서버로 설정한다. getList 함수는 axios.get 함수를 사용하여 HTTP GET 메소드를 호출한다. useEffect 내부를 살펴보면 getList 함수를 호출한다. 성공적인 경우 setList 함수를 호출하여 응답으로 전달된 데이터를 저장한다. 데이터 저장 시 slice를 이용해 최신 20개 데이터만 조회</Text>
                    </Flex>
                    <Image 
                        styles={{
                            width:"332px",
